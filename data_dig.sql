...
INSERT INTO marvels VALUES(11, "Jean Gray", 8, "Good", "Female", 1.68, 52.16, "USA", 3, 2, 7, 7, 7, 4);
...
SELECT * FROM marvels;
SELECT MAX(height_m) AS "max_height", MIN(height_m) AS "min_height" FROM marvels;
SELECT alignment, gender, COUNT(alignment) AS alignment_count FROM marvels
    GROUP BY alignment, gender
    HAVING gender = "Female"
    ;
SELECT name, popularity,
    CASE
        WHEN popularity < 10 THEN "Fan Favorites"
        WHEN popularity < 50 THEN "Generally loved"
        WHEN popularity < 80 THEN "Not as important"
        ELSE "Most hated"
    END as "fan _feelings"
    FROM marvels
    ORDER BY popularity
    ;
